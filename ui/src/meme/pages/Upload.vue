...
<template>
    <div
        class="cover fullScreenUpload flex"
        v-if="show_drag || file"
        @dragleave="
            () => {
                show_drag = false;
    ...
    >
        ...
        <div v-if="file" class="p-4 m-4 flex flex-col gap-6 max-h-[calc(90dvh)]">
            <img :src="source" class="max-h-[calc(60dvh)] max-w-[calc(90dvw)] object-contain" />
            <div class="grid items-center align-middle place-items-center">
                <input
                    type="text"
                    placeholder="tags"
                    class="input input-sm w-[calc(80dvw)] bg-transparent m-5 text-center border-none"
                    v-model="tags"
                />
